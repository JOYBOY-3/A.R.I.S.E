...
const unsigned long WIFI_RECONNECT_INTERVAL = 10000;  // Try reconnect every 10 seconds
 ...
 * Ensures each roll is queued only once per offline period.
 * @param roll_id: Student roll ID to queue
 */
void addToQueue(int roll_id) {
  String subMsg = "Roll #" + String(roll_id);

  // Check if this roll is already in the offline queue
  for (int i = 0; i < queueCount; i++) {
    if (offlineQueue[i] == roll_id) {
      Serial.println("[QUEUE] Roll already queued offline: " + String(roll_id));
  ...
  // Add new roll to queue if space available
  if (queueCount < MAX_QUEUE_SIZE) {
    offlineQueue[queueCount] = roll_id;
    queueCount++;
    Serial.println("[QUEUE] Saved offline roll: " + String(roll_id) + " (" + String(queueCount) + " total)");
    updateDisplay("Saved Offline", subMsg);
  } else {
    Serial.println("[QUEUE] Queue FULL, cannot save roll: " + String(roll_id));
    updateDisplay("Queue FULL", subMsg);
  }
  delay(2000);
}
 ...
 * Uses bulk endpoint to send all queued roll IDs in one request
 */
void tryToSyncQueue() {
  ...
  int originalQueueCount = queueCount;
  updateDisplay("Syncing Queue", String(queueCount) + " rolls...");
  delay(1000);
  ...
  JsonDocument doc;
  JsonArray rollArray = doc["roll_ids"].to<JsonArray>();
  for (int i = 0; i < queueCount; i++) {
    rollArray.add(offlineQueue[i]);
  }

  String jsonPayload;
  serializeJson(doc, jsonPayload);
  Serial.println("[QUEUE] Bulk sync payload: " + jsonPayload);
  ...
  if (httpCode == 200) {
    ...
    if (!error) {
      int successCount = responseDoc["success_count"] | 0;
      JsonArray failedArray = responseDoc["failed"].as<JsonArray>();

      // Rebuild queue with only failed IDs
      int newQueueCount = 0;
      if (!failedArray.isNull()) {
        for (JsonVariant v : failedArray) {
          int failedRoll = v.as<int>();
          if (newQueueCount < MAX_QUEUE_SIZE) {
            offlineQueue[newQueueCount++] = failedRoll;
          }
        }
      }
      ...
      Serial.println("[QUEUE] Bulk sync complete: " + String(successCount) + "/" + String(originalQueueCount) + " OK");
      if (queueCount > 0) {
        Serial.println("[QUEUE] Remaining failed IDs in queue: " + String(queueCount));
      }

      updateDisplay("Queue Synced", String(successCount) + "/" + String(originalQueueCount) + " OK");
      delay(1500);
    } else {
      Serial.println("[QUEUE] Bulk sync JSON parse error");
    }
  } else {
    Serial.println("[QUEUE] Bulk sync HTTP error: " + String(httpCode));
  }
...
void setup() {
  ...
  // Enable station mode and automatic reconnection
  WiFi.mode(WIFI_STA);
  ...
  // Initialize network timers so first checks happen after their intervals
...
void loop() {
  ...
  // WiFi auto-reconnect management (non-blocking)
  if (WiFi.status() != WL_CONNECTED) {
